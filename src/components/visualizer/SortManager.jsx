...
export const SortManager = React.memo(function ({
  ...
  async function reset() {
    ...
    sortProgressIterator.current =
      sortingAlgorithmName == "LinearSearch" || "BinarySearch"
        ? await sortFunction(algoArray.current, element, highlight, markSort)
        : await sortFunction(algoArray.current, swap, highlight, markSort);

    sortProgressIterator.current =
      sortingAlgorithmName == "BubbleSort" || "SelectionSort"
        ? await sortFunction(algoArray.current, swap, highlight, markSort)
        : await sortFunction(algoArray.current, swap, highlight, markSort);
  ...
  function element() {
    return useData.getState().searchingElement;
  }
